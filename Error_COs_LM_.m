...
function Error=Error_GOs(GOs,NUMBER_LRFs,FixedLRF,CalibLRF,R,T)
nGOs=size(GOs,2);
errors=zeros(1,nGOs);
...
%                         vNormal=[plane(1);plane(2);plane(3)]; vNormal=vNormal./norm(vNormal);
                    ...
                    errors(iGO)=errors(iGO)+error_planarity^2;
                end
            end
        end
    end
    ...
    for iWall=1:4
        ...
        cntLines1=GOs(iGO).GO(iWall).cntLines;
        lines1=GOs(iGO).GO(iWall).lines;
        cntLines2=GOs(iGO).GO(mod(iWall,4)+1).cntLines;
        lines2=GOs(iGO).GO(mod(iWall,4)+1).lines;
...
%                         vNormal2=[plane2(1);plane2(2);plane2(3)]; vNormal2=vNormal2./norm(vNormal2);
        ...
        errors(iGO)=errors(iGO)+error_orthogonality^2;
    end
    % end of oneGO
end

Error=sum(errors)/nGOs;
